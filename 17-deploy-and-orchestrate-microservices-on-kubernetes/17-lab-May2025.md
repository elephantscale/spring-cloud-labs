...
#### 8.3  **Visually confirm all three replicas are ready**

```powershell
$svc='user-service','order-service'; foreach($d in $svc){$avail=(kubectl get deploy $d -o jsonpath='{.status.availableReplicas}'); if(!$avail){$avail=0}; $desired=(kubectl get deploy $d -o jsonpath='{.spec.replicas}'); $ip=(kubectl get svc $d -o jsonpath='{.spec.clusterIP}'); $ports=(kubectl get svc $d -o jsonpath='{range .spec.ports[*]}{.port}:{.targetPort}{" "}{end}'); $nodes=(kubectl get pods -l app=$d -o jsonpath='{range .items[*]}{.metadata.name}=>{.spec.nodeName}{"; "}{end}'); $msg="{0,-14} {1}/{2} replicas  IP:{3,-15} Ports:{4,-10} Nodes:{5}" -f $d,$avail,$desired,$ip,$ports.Trim(),$nodes.Trim('; '); if([int]$avail -eq [int]$desired){Write-Host \"✔ $msg\" -ForegroundColor Green}else{Write-Host \"✖ $msg\" -ForegroundColor Red}}
```


### 8.3 **Validate all three instances are running** (New PowerShell window)
...
1. **Watch the pods**
   ...
   ```
   Wait until **three** pods show `Running`, then **Ctrl +C**.

2. **Check deployment counts**

   ```powershell
   kubectl describe deployment order-service | findstr /C:"Replicas"
   ```
   Expected: `3 desired | 3 updated | 3 available`.

3. *(Optional)* **Wide view**
