...
```powershell
# One-liner: prints a green ✔ if all replicas are ready, red ✖ otherwise
$replicas = kubectl get deploy order-service `
            -o jsonpath='{.status.availableReplicas}/{.spec.replicas}'

$avail, $total = $replicas -split '/'

if ($avail -eq $total) {
    Write-Host "✔ $replicas" -ForegroundColor Green
} else {
    Write-Host "✖ $replicas" -ForegroundColor Red
}
```
