...
```powershell
# one-liner: prints a green check-mark if 3/3 replicas are available
printf "Order Service Replicas: "; \
kubectl get deploy order-service -o jsonpath='{.status.availableReplicas}/{.spec.replicas}' | \
while IFS=/ read avail total; do \
  if [ "$avail" = "$total" ]; then \
