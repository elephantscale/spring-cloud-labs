...
```powershell
$svc='user-service','order-service'; foreach($d in $svc){$r=(kubectl get deploy $d -o jsonpath='{.status.availableReplicas}/{.spec.replicas}'); $ip=(kubectl get svc $d -o jsonpath='{.spec.clusterIP}'); $a,$t=$r -split '/'; if($a -eq $t){Write-Host "✔ $d  $r  (IP $ip)" -ForegroundColor Green}else{Write-Host "✖ $d  $r  (IP $ip)" -ForegroundColor Red}}
```
